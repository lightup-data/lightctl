...
class SourceClient(BaseClient):
    ...
    def sources_url(self):
        return urllib.parse.urljoin(URL_BASE, "/api/v1/sources/")
    ...
    def inspect(self, data):
        url = urllib.parse.urljoin(URL_BASE, "/api/v1/sources-inspection")
        return self.post(url, data, expected_status=200)

    def list_tables(self, id):
        url = urllib.parse.urljoin(URL_BASE, f"/api/v1/sources/{id}/tables")
        return self.get(url)

    def get_schema(self, id, table_name):
        url = urllib.parse.urljoin(
            URL_BASE, f"/api/v1/sources/{id}/schema?table_name={table_name}"
        )
        return self.get(url)
